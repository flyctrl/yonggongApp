...
export const parseJsonUrl = (ojson) => { // json对象转URL
  ...
  for (let p in ojson) {
    ...
    key = ojson[p]
    s += '&' + name + '=' + encodeURIComponent(key.toString())
  }
...
export const parseURLParam = (url) => { // 把url的参数部分转化成json对象
  try {
    let targeturl = url || (history.location.pathname + history.location.search)
    const regUrl = /^[^\?]+\?([\w\W]+)$/
    const regPara = /([^&=]+)=([\w\W]*?)(&|$|#)/g
...
export const queryTourl = function (backquery) {
  /*
    &backquery=a:1,b:1,c:1
  */
  let queryAry = backquery.split(',')
  let queryStr = ''
  queryAry.map((item) => {
    queryStr += item.split(':')[0] + '=' + item.split(':')[1] + '&'
  })
  queryStr = '?' + queryStr.slice(0, queryStr.length - 1)
  return queryStr
}

export const getPosition = (timeout = 10000, maximumAge = 60000, enableHighAcuracy = false) => new Promise((resolve, reject) => {
